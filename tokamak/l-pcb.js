...
const coil = nTurns => {
    ...
    const res = turns.reduce((res, turn) => {
        const {root, pos} = res;
        return {
...
const windedToroid = () => union(
    rotate_extrude({fn: 72},
        translate([20.7, 0, 0],
            t200CrossSection()
        )
    ).setColor([1, 1, 0, 0.5]),
    coil(3 + 7/12).setColor([1, .3, .1])
);
...
const pcb = () => union(
    cube([80, 31, 1.6]).setColor([0, .3, 0]).translate([-20, -15.5, -.8]),
    union(
        translate([-6, 0, 0], relay()),
        translate([ 6, 0, 0], relay())
    ).translate([0, 0, .8]),
    rotate([180, 0, 0],
        union(
...
const main = () => union(
    rotate([0, 90, 0], windedToroid()),
    pcb()
);

/* global union cube translate rotate_extrude rotate cylinder polygon */
